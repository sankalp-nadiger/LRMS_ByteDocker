...
interface LandRecordTimelineProps {
  landId?: string;
  showHeader?: boolean;
}
...
export default function LandRecordTimelinePage({ landId, showHeader = true }: LandRecordTimelineProps) {
  const { user } = useUser();
  const role = useUserRole();
  
  const [lands, setLands] = useState<LandRecord[]>([]);
  const [selectedLandId, setSelectedLandId] = useState<string>(landId || '');
  ...
  useEffect(() => {
    if (landId) {
      ...
      const paramLandId = params.get('landId');
      if (paramLandId) {
  ...
  // Fetch land records (only if landId prop is not provided)
  useEffect(() => {
    if (landId) return; // Skip if landId prop is provided
 ...
 // Set selected land when selectedLandId changes
  useEffect(() => {
    ...
    } else if (selectedLandId && !selectedLand) {
      // If we have selectedLandId but no selectedLand, fetch it directly
      const fetchSelectedLand = async () => {
        try {
          const { data, error } = await supabase
            .from('land_records')
            .select('*')
          ...
          console.error('Error fetching selected land:', err);
        }
      };
      fetchSelectedLand();
    }
            ...
            {selectedLand && (
              <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                <h2 className="text-xl font-semibold mb-2">
                  {selectedLand.village}, {selectedLand.taluka}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                  <div>
                    <span className="font-medium">District:</span> {selectedLand.district}
                  </div>
                  <div>
                    <span className="font-medium">Block No:</span> {selectedLand.block_no}
                  </div>
                  <div>
                    <span className="font-medium">Status:</span> 
                    <span className="ml-1 capitalize">{selectedLand.status}</span>
                  </div>
                </div>
              </div>
            )}
