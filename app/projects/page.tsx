...
export default function ProjectsPage() {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      // Refresh data to update available land records
      await fetchData();
      ...
      for (const project of clusterProjects) {
        ...
        if (projectLandRecords.length > 0) {
          projectLandRecords.forEach((record, index) => {
            exportData.push({
              projectName: index === 0 ? project.name : '',
  ...
  const openDeleteConfirm = () => {
    if (selectedProject && !selectedProject.is_individual) {
      ...
      alert('Individual land records cannot be deleted from here. Please delete from Land Master.');
    }
  };
              ...
              <div className="mb-6">
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                  <p className="text-sm text-yellow-800">
                    <strong>Note:</strong> This will only delete the project cluster. 
                    The {selectedProject.land_record_ids?.length || 0} land record(s) in this project 
                    will remain in the system and become individual projects.
                  </p>
                </div>
              </div>
